...
public class PaymentPlanAdminController {
    ...
    public ResponseEntity<PaymentPlan> createPlan(@Valid @RequestBody CreatePlanRequest request) {
        ...
        // Handle Square customer/card creation if cardToken is provided
        if (request.cardToken() != null && !request.cardToken().isBlank()) {
            try {
                var squareService = service instanceof com.example.simpletixwebhook.service.PaymentPlanService psvc && psvc != null
                        ? psvc.getSquarePaymentService()
                        : null;
                if (squareService == null && service instanceof com.example.simpletixwebhook.service.PaymentPlanService psvc2) {
                    java.lang.reflect.Field f = psvc2.getClass().getDeclaredField("squarePaymentService");
                    f.setAccessible(true);
                    squareService = (com.example.simpletixwebhook.service.SquarePaymentService) f.get(psvc2);
                }
                if (squareService == null) {
                    throw new IllegalStateException("SquarePaymentService not available");
                }
                var result = squareService.createCustomerAndCard(request.email(), request.cardToken());
                plan.setSquareCustomerId(result.customerId());
                plan.setSquareCardId(result.cardId());
            } catch (Exception ex) {
                log.error("Failed to create Square customer/card for plan", ex);
