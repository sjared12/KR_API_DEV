...
public class RefundService {
        /**
         * Request a refund for a payment plan
         */
        public SubscriptionRefund requestRefund(PaymentPlan paymentPlan, BigDecimal requestedAmount,
            ...
            if (requestedAmount.compareTo(paymentPlan.getTotalDue()) > 0) {
                throw new IllegalArgumentException("Refund amount cannot exceed plan total due");
            }

            SubscriptionRefund refund = new SubscriptionRefund(paymentPlan, requestedAmount, reason, requestedByUser);
            refund.setStatus(RefundStatus.REQUESTED);

            SubscriptionRefund saved = refundRepository.save(refund);
            log.info("Refund requested: {} for payment plan {}", saved.getId(), paymentPlan.getId());
            return saved;
        }
